...
public class PathFinder : MonoBehaviour {
	...
	void Awake() {
		...
		while (coords.Count > 0) {
			...
			if (builder.parent != null && _collision(newNode.x, newNode.z)) {
			...
			bool nX = _collision(newNode.x - 1, newNode.z) == false;
			bool pX = _collision(newNode.x + 1, newNode.z) == false;
			bool nZ = _collision(newNode.x, newNode.z - 1) == false;
			bool pZ = _collision(newNode.x, newNode.z + 1) == false;
			...
			if (nX || nZ) {
				...
				coords.Push(new _PathNodeBuilder(newNode, new PathNode(newNode.x - 1, newNode.z - 1)));
			}
			if (nX || pZ) {
